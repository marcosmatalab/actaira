...
| D-306 | Nothing is uploaded to a package index: the tool installs from its release tag and the release carries the attested wheel and sdist, because a publishing job with no publisher registered can only fail | `scripts/release_check.py:2250` |
| D-307 | A command the parser already has may not be written up under `## Unreleased`, because a reader of a tag has to be able to tell from the changelog what it contains | `scripts/release_check.py:2186` |
| D-308 | A hint for an unread scope names `seamark check --machine` rather than a bare flag, because the causes print under `check` and `diff` and only `check` takes it | `src/seamark/surface/cursor.py:132` |
