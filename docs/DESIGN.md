...
| D-25b | The same question at the package boundary | `src/actaira/attest/verify.py:191` |
...
| D-235 | A reader that stops reading is a shell convention, not an error, and never a traceback over a success code | `src/actaira/cli.py:1065` |
...
| D-240 | The default key path is resolved when a parser is built, not at import, so a host with no home does not break every command | `src/actaira/cli.py:77` |
| D-241 | What this tool prints is UTF-8 when it is redirected, because the locale is not something a report should depend on | `src/actaira/cli.py:1032` |
...
| D-262 | A session id is a string out of somebody's file, so it is sanitised and a collision is numbered, never overwritten | `src/actaira/cli.py:797` |
...
| D-271 | Every read of somebody else's repository is bounded before it is attempted, and every path is resolved before it is opened | `src/actaira/surface/disk.py:15` |
| D-272 | Finding the script a hook names is a parse, and a shape it cannot parse costs a stated gap rather than a guess | `src/actaira/surface/disk.py:301` |
...
| D-276 | Exit code 3 comes back, because a pipeline branching on 1 alone reads "I could not tell" as a clean run | `src/actaira/cli.py:57` |
| D-277 | The agent's version is never obtained by running the agent, because asking the audited tool what it is is trusting it | `src/actaira/cli.py:371` |
| D-278 | The rule page is generated, so it cannot be the stale fourth copy of what a rule says | `scripts/rules_doc.py:4` |
| D-279 | A path that is absolute on another platform is absolute here too, or the same repository gets two answers on two machines | `src/actaira/surface/disk.py:143` |
| D-280 | JSONC forgives comments and trailing commas and nothing else, because a permissive parser is the fail-open shape of a configuration reader | `src/actaira/surface/jsonc.py:9` |
| D-281 | Frontmatter is read as a named YAML subset that refuses the rest, rather than grepped for the keys a rule wants | `src/actaira/surface/miniyaml.py:9` |
| D-282 | A `folderOpen` task is the phase's canonical declared-versus-effective case, and the documented default is not the answer when nobody read the scope that sets it | `src/actaira/surface/vscode.py:8` |
| D-283 | `initializeCommand` is read separately from the other five because it runs on the host, outside the containment that is the point of a container | `src/actaira/surface/devcontainer.py:3` |
| D-284 | Codex's project layer waits on a trust level that lives in the user's file, so a repository hook is declared until that file is read | `src/actaira/surface/codex.py:6` |
| D-285 | Cursor's team level is reported INDETERMINATE on every run, because a dashboard-synced hook is outside the filesystem by design | `src/actaira/surface/cursor.py:3` |
| D-286 | Each vendor carries its own precedence ladder, because Gemini's inverts Claude Code's and one table would have to invent a winner | `src/actaira/surface/gemini.py:3` |
| D-287 | Instruction files are read for structure and literals only, because classifying what free text means is judging intention | `src/actaira/surface/instructions.py:3` |
| D-288 | A repository's surface is the union of per-vendor surfaces and never a merge, so two vendors running one command stay two capabilities | `src/actaira/surface/resolve.py:311` |
| D-289 | `check` is exposed over MCP only once it reads more than one vendor, because `tools/list` is read as capability and cannot be corrected later | `src/actaira/mcp.py:80` |
| D-290 | A capability name is a literal in our source, never assembled from the file being read, or the audited repository writes Actaira's vocabulary | `src/actaira/surface/claude_code.py:91` |
| D-291 | A setting in a vendor's superseded spelling is read and left INDETERMINATE, because inventing the migration and inventing its removal are both inventions | `src/actaira/surface/gemini.py:53` |
| D-292 | Every capability this tree emits is named by a rule or excused in writing, because a capability no rule can fire on is a line nobody can act on | `src/actaira/surface/resolve.py:1912` |
| D-293 | A ref's tree is materialised whole rather than filtered to the paths the readers know, or `diff` and `check` answer about two different trees | `src/actaira/surface/diff.py:21` |
| D-294 | Two refs are read with `ls-tree` and `cat-file` and never checked out, because a checkout writes in the operator's tree and runs their hooks | `src/actaira/surface/diff.py:9` |
| D-295 | Widened and narrowed exist only where the fact's own name states which value is the wider one; everything else is CHANGED with both digests | `src/actaira/surface/diff.py:370` |
| D-296 | A seal carries one digest per facts mapping rather than one per fact, because a per-fact digest is a dictionary attack somebody else already has the words for | `src/actaira/attest/seal.py:14` |
| D-297 | The HTML report loads nothing over the network, so a security report cannot phone home while it is being read | `src/actaira/report/html.py:7` |
...
| D-301 | Every claim a README makes about a command, a flag or an exit code resolves against the parser, and a claim block that names no command is refused | `scripts/release_check.py:649` |
