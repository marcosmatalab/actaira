...
| D-235 | A reader that stops reading is a shell convention, not an error, and never a traceback over a success code | `src/actaira/cli.py:1067` |
...
| D-240 | The default key path is resolved when a parser is built, not at import, so a host with no home does not break every command | `src/actaira/cli.py:78` |
| D-241 | What this tool prints is UTF-8 when it is redirected, because the locale is not something a report should depend on | `src/actaira/cli.py:1034` |
...
| D-262 | A session id is a string out of somebody's file, so it is sanitised and a collision is numbered, never overwritten | `src/actaira/cli.py:799` |
...
| D-273 | Every merge decision is a cited row, anchored to a digest of the page because these pages publish no version | `src/actaira/surface/merge.py:3` |
...
| D-276 | Exit code 3 comes back, because a pipeline branching on 1 alone reads "I could not tell" as a clean run | `src/actaira/cli.py:58` |
| D-277 | The agent's version is never obtained by running the agent, because asking the audited tool what it is is trusting it | `src/actaira/cli.py:372` |
...
| D-288 | A repository's surface is the union of per-vendor surfaces and never a merge, so two vendors running one command stay two capabilities | `src/actaira/surface/merge.py:312` |
...
| D-291 | A setting in a vendor's superseded spelling is read and left INDETERMINATE, because inventing the migration and inventing its removal are both inventions | `src/actaira/surface/gemini.py:62` |
| D-292 | Every capability this tree emits is named by a rule or excused in writing, because a capability no rule can fire on is a line nobody can act on | `src/actaira/surface/resolve.py:482` |
