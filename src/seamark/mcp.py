...
recording rather than quietly making. Two unbuilt tools, `seamark_contract`
and `seamark_verdict`, were announced in `tools/list` and answered every call
...
`seamark_verify` is real, and is the existing offline verifier with nothing
...
def _verify(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(path, str) or not path:
        return _text(
            {"state": "usage_error", "detail": "seamark_verify needs a `path` to a package"},
            is_error=True,
        )
...
def _check(arguments: dict[str, Any]) -> dict[str, Any]:
    """`seamark check` over MCP, answering with the bytes the command prints.
    ...
    if not isinstance(spoken, str) or not spoken:
        return _text(
            {"state": "usage_error", "detail": "seamark_check needs a `repo` directory"},
            is_error=True,
        )
    repo = Path(spoken)
    if not repo.is_dir():
...
TOOLS: dict[str, dict[str, Any]] = {
    ...
    "seamark_verify": {
        "description": (
            "Verify an Seamark attestation package offline and report its state. "
            "Nothing is scored."
        ),
        "inputSchema": {
            "type": "object",
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "seamark", "version": __version__},
            },
        }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv:
        print("seamark-mcp takes no arguments: it speaks JSON-RPC on stdin.", file=sys.stderr)
        return 2
    return serve()
