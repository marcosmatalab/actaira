...
def default_key_path() -> Path:
    ...
    Design note D-240. This was a module-level `Path.home() / ".seamark" /
    ...
    try:
        ...
        return Path(".seamark") / "signing-key.pem"
    return home / ".seamark" / "signing-key.pem"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"seamark {__version__}")
    ...
    seal.add_argument("--key", type=Path, default=default_key_path(),
                      help="the Ed25519 key `seamark keygen` wrote")
    ...
    watch.add_argument("--out", type=Path, default=Path("seamark-trace"),
    ...
    # so `seamark watch -- claude ...` dispatched to `verify`. argparse gives no
...
def run_diff(args: argparse.Namespace, catalog: Catalog) -> int:
    ...
    if named_dirs:
        ...
        if not (args.repo / ".git").exists():
            ...
            return EXIT_USAGE
        with tempfile.TemporaryDirectory(prefix="seamark-diff-") as scratch:
            try:
                sides = [
...
def _settle_output_encoding() -> None:
    ...
    install - `seamark --lang es verify ... > out.txt` wrote cp1252 bytes and
...
def main(argv: list[str] | None = None) -> int:
    ...
    Design note D-235. `seamark verify --json | head -3` printed a
