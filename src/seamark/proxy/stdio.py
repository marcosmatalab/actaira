...
Run as a module by `seamark watch`, which rewrites the agent's `.mcp.json` to
point at `python -m seamark.proxy.stdio --record <file> -- <original command>`.
"""
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover - driven as a process
    parser = argparse.ArgumentParser(prog="seamark.proxy.stdio")
