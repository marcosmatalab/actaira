"""DSSE envelopes and in-toto statements, so an Seamark report can be read by
tools that never heard of Seamark.
...
`sigstore/model-transparency` is its reference implementation. Seamark's own
...
cannot check an Seamark zip, and the cost of that is not theoretical: it is
...
model signature and an Seamark report are different objects: OMS and
...
publisher is known, not a model that is safe. Seamark never says who signed
...
that the signature was supposed to cover and does not. Seamark's own package
...
No command reached any of them: `seamark verify` enters this module at
...
def verify_envelope(envelope: Envelope, public_key: Ed25519PublicKey) -> tuple[bool, list[str]]:
    ...
    The checks, in the order they are listed in the output:
      ...
      6. the predicate type is Seamark's. A foreign predicate is a valid
         in-toto statement and this function still calls it a problem,
         because its contract is "is this an Seamark inspection statement I
