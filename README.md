...
**Open-source security and governance console for AI models and agents.**
...
**The engine is ahead of the interface, and that is the next thing to close.** The CLI reaches every capability above; the local interface covers inspection, agents, policy, the asset graph, impact, attestation, verification and governance. Bundles, controls, discovery, sources, watch, snapshots, evidence, trust and receipts are reachable from the terminal only today. The graph panel reads what `watch` and `evidence` wrote, and that is not the same as reaching them: it shows their output and cannot register a source, run an observation or open a snapshot. Which of them is which is not prose: `tests/test_cli_ui_parity.py` records it per capability and fails if a row and the routes disagree, in either direction.
