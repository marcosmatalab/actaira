...
|---|---|---|
| **2,704 tests**, run on Python 3.11, 3.12 and 3.13 | **32 documented rules** from versioned packs | **7 CLI commands**, one runtime dependency |
...
|---|---|---|
| 2,704 tests | `python -m pytest --collect-only -q -o addopts=` | the same count |
| 15,989 lines of product code | `find src -name '*.py' \| xargs cat \| wc -l` | the same count |
| 43,442 lines of Python in the tree | `python scripts/figures.py` | `docs/FIGURES.md`, per area |
