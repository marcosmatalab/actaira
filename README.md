...
**Actaira 3.0.0** · Python 3.11 · 3.12 · 3.13 · MIT · one runtime dependency · local-first, no telemetry, no account
...
| **1,974 tests**, none of them hand-written figures | **7 connectors** that never decide | one runtime dependency |
...
**17 policy predicates** over **5 kinds of subject**, one language, ALLOW / DENY / REVIEW. A predicate with no information goes to REVIEW rather than quietly returning false, and the decision carries the rules and evidence that caused it.
...
**36,441 lines of Python**, one runtime dependency, and **84 design notes** recording why each decision went the way it did. [`docs/DESIGN.md`](docs/DESIGN.md) is the index; every note names the file and line that implements it, and a test fails if a note in the code is missing from the table.
